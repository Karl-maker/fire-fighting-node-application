...
//     const res = await request(app).get(`${IOT_DEVICE_ROUTE}?pageNumber=2&pageSize=5`);
//     expect(res.status).toBe(200);
...
//     const res = await request(app).get(`${IOT_DEVICE_ROUTE}?filters[name][exact]=Device Alpha`);
//     expect(res.status).toBe(200);
//     expect(res.body.data).toHaveLength(1);
...
//     await iotDeviceRepository.save(
//       new IotDevice({ id: null, name: "Beta Sensor", firmwareVersion: "", type: DeviceType.VALVE, status: DeviceStatus.OFFLINE, createdAt: new Date(), updatedAt: new Date(), lastSeenAt: null, ipAddress: null, position: null, location: null, metadata: {} })
//     );

//     const res = await request(app).get(`${IOT_DEVICE_ROUTE}?filters[name][contains]=Sensor`);
//     expect(res.status).toBe(200);
...
//     const res = await request(app).get(`${IOT_DEVICE_ROUTE}?filters[status][exact]=ONLINE`);
//     expect(res.status).toBe(200);
//     expect(res.body.data).toHaveLength(1);
...
//     const res = await request(app).get(`${IOT_DEVICE_ROUTE}?sortBy=name&sortOrder=asc`);
//     expect(res.status).toBe(200);
